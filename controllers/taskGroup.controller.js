const { isProjectMember } = require("../services/projectMember.service");
const {
...
class TaskGroupController {
  static async getGroupsByGroupBy(req, res) {
    try {
      ...
      if (!isProjectMember(req.params.projectId, req.decoded.userId) || !groups) res.status(400).json({ message: "bad request" });
  ...
  static async deleteTaskGroup(req, res) {
    try {
      ...
      if (deletedGroup === "NOT_EMPTY")
        res.status(400).json({ message: "group is not empty" });
